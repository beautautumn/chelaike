...
class StockOutInventory < ActiveRecord::Base
  ...
  validates :completed_at, presence: true, if: :sold?
  validates :seller_id, presence: true, if: :sold?
  validates :customer_channel_id, presence: true, if: :sold?
  validates :closing_cost_cents, presence: true, if: :sold?
  validates :sales_type, presence: true, if: :sold?
  validates :payment_type, presence: true, if: :sold?
  ...
  enumerize :sales_type, in: %i(retail wholesale)
