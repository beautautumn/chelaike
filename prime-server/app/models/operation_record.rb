...
class OperationRecord < ActiveRecord::Base
  ...
  enumerize :operation_record_type,
            in: %i(
              ...
              accredited_updated debit_updated replace_cars_updated
              system_announcement first_reported verified unverified
              first_check_appointment
            ),
            groups: {
              cars: %i(
              ...
              loan_messagable: %i(
                loan_bill_state_updated accredited_updated
                debit_updated replace_cars_updated
              )
            }
  ...
  ransacker :funder_company_name do |parent|
    Arel::Nodes::InfixOperation.new(
      "->>", parent.table[:messages], Arel::Nodes.build_quoted("funder_company_name")
    )
  end
  ...
  def loan_message_text
    extra_params = case operation_record_type
                   ...
                   when "replace_cars_updated"
                     {
                       loan_bill_code: messages["loan_bill_code"],
                       original_cars: messages["original_cars"],
                       new_cars: messages["new_cars"]
                     }
                   end
