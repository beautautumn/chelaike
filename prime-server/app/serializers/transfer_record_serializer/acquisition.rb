...
module TransferRecordSerializer
  class Acquisition < ActiveModel::Serializer
    include SerializerAuthorityHelper
    attributes :id, :car_id, :state, :state_text, :transferred_at, :vin,
               :items, :contact_person, :contact_mobile,
               ...
               :user_name, :user_id, :inspection_state, :compulsory_insurance_end_at,
               :annual_inspection_end_at, :commercial_insurance_end_at,
               :commercial_insurance_amount_yuan, :note, :created_at,
               :data_completeness, :commercial_insurance, :compulsory_insurance,
               :current_location_province, :current_location_city, :new_plate_number,
               :new_owner, :new_owner_idcard, :new_owner_contact_mobile
    ...
    def attributes(requested_attrs = nil, reload = false)
      ...
      unless scope.id == object.try(:user_id) || authority_filter("牌证信息查看")
        attrs.except!(:original_owner, :original_owner_idcard)
      end

      unless scope.id == object.try(:user_id) || authority_filter("牌证图片查看")
        attrs.except!(:images)
      end
