...
module UserSerializer
  class Common < ActiveModel::Serializer
    attributes :id, :username, :phone, :authority_type, :authorities, :created_at,
               :avatar, :name, :state, :is_alliance_contact, :note, :email,
               :mac_address, :device_numbers, :cross_shop_authorities,
               :shop_id, :settings, :first_letter, :cross_shop_read_statistic,
               :mobile_app_car_detail_menu, :qrcode_url, :self_description

    has_many :authority_roles, serializer: AuthorityRoleSerializer::Common
    has_one :manager, serializer: Basic
    has_one :shop, serializer: ShopSerializer::Common
    has_one :company, serializer: CompanySerializer::Basic
  end
end
