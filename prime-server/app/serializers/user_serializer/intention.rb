...
module UserSerializer
  class Intention < ActiveModel::Serializer
    attributes :id, :username, :phone, :authority_type, :authorities, :created_at,
               ...
               :shop_id, :settings, :intentions_count, :first_letter,
               :cross_shop_read_statistic

    def intentions_count
      key = "#{self.class.name}:users:#{object.cache_uuid}"

      Rails.cache.fetch(key) do
        object.active_intentions.size
      end
    end
  end
end
