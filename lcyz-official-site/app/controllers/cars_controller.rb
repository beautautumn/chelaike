...
class CarsController < ApplicationController
  ...
  def show
    ...
    @detection_report = Chelaike::CarService.detection_report(@car.id, @tenant.company_id)
    ...
    @alliance_name = alliance_name(@tenant.company_id)

    @alliance_similar_count = Chelaike::CarService.similar_count({
                                                                   car_id: params[:id],
                                                                   per_page: 999,
                                                                   search_range: "company_except"
                                                                 }, @tenant.company_id).to_i
    @seller = current_seller
  ...
  def brand_and_series
    all_brands = Chelaike::BrandService.fetch_brands(@tenant.company_id)
    ...
    all_series = Chelaike::BrandService.fetch_series(nil, @tenant.company_id, true)
  ...
  def inspection_report
    detection = Chelaike::CarService.detection_report(params[:id], @tenant.company_id).data
  ...
  def load_desktop_brands
    ...
    @series = Chelaike::BrandService.fetch_series(brand_name, @tenant.company_id)
  end
  ...
  def init_params
    ...
    param! :query, Hash
    param! :page, Integer, default: 1
    param! :without_allied, String, default: "true" if @tenant.tenant_type == "solo"
  end
  ...
  def alliance_main_site(company_id)
    owner_id = alliance_main_site_info(company_id).try(:owner_id)
    return unless owner_id
    owner_tenant = Tenant.where(company_id: owner_id).first
    owner_tenant.full_url
  end

  def alliance_name(company_id)
    alliance_main_site_info(company_id).try(:alliance_name)
  end

  def alliance_main_site_info(company_id)
    @_alliance_info ||= Chelaike::CompanyService.alliance_owner_info(company_id)
  end
end
