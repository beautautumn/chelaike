...
function Acquisition({ car, usersById }) {
  if (!can('收购信息查看', null, car.shop)) {
    return <span />
  }
  const [acquisitionPriceLabel, acquisitionPrice] = acquisitionPriceText(car)
  const data = [
    { label: '车源负责人：', text: car.acquirer && usersById[car.acquirer].name },
    { label: '电话：', text: car.acquirer && usersById[car.acquirer].phone },
  ]
...
export default class List extends Component {
  ...
  render() {
    ...
    return (
      <Segment className={styles.carsList}>
        <Helmet title="在库车辆" />
