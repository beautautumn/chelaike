...
<script>
	...
	$(function(){
		$("#startDate").val('${normalContinueStartDate}');
		setEndDate();
	...
	function openSiteArea(){
		...
		menu.create_child_dialog_identy(api,W,'getContinueSitePage','选择场地区域',
    		'/rent/contractListAction!toContinueSelectSiteArea.action?areaNoAll='+areaNoAll+'&currentSiteAreas='+currentSiteAreas,
    		400,350,true);
    }
	...
	function do_submit(){
		...
		if(confirm("确认保存合同？")){
			...
			$.ajax({
					cache: false,
					type: "POST",
					url:ctutil.bp()+"/rent/contractListAction!continueContract.action",
					data:data,
					async: false,
					error: function() {
					html = "数据请求失败";
					...
					success: function(data) {
					  if(data=="success"){
						  alert("保存成功！");
						  api.get("continueContractPage", 1).close();
						  W.query1();  //刷新副页面
					  }else{
						  alert("保存失败！");
						  api.get("continueContractPage", 1).close();
