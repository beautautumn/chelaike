...
<script>
	...
	function do_submit(){
		...
		if(recvDepositFee==''){
			alert("本次实收押金   不能为空！");
			return;
		}else if(recvDepositFee-depositFee<0){
			alert("本次实收押金   必须大于等于   本次应补押金！");
			return;
		}
		...
		data="shopContractId="+$("#contractId").val()
			+"&contractCollectionPlanBean.recvFee="+recvFee
			+"&contractCollectionPlanBean.recvDepositFee="+recvDepositFee
			+"&contractCollectionPlanBean.recvDate="+recvDate
			+"&contractCollectionPlanBean.staffId="+staffId
			+"&contractCollectionPlanBean.remark="+remark
			+"&contractCollectionPlanBean.planId="+cycleId;
		
		$.ajax({
				cache: false,
				type: "POST",
				/* url:ctutil.bp()+"/rent/contractPaymentAction!doRectFee.action", */
				url:ctutil.bp()+"/rent/shopContractAction!doRecvFee.action",
				data:data,
				async: false,
				error: function() {
				html = "数据请求失败";
...
<div class="box">
			<table>	
					<input type="hidden" class="input w200" id="contractId" name="shopContract.id" value="${shopContract.id}"/>
					<input type="hidden" id="cycleId" name="contractCollectionPlanBean.planId" value="${collectionPlan.id }" />
				<tr>
					<td class="text-r">应收总额：</td>
					<td><input class="input w200" id="planCollectionFee"  readonly="readonly" style="background-color:#F0F0F0" value="${collectionPlan.planCollectionFee/100 }"/>元</td>
				</tr>
				<tr>
					<td class="text-r">商户已交金额：</td>
					<td><input class="input w200"  readonly="readonly" style="background-color:#F0F0F0" value="${collectionPlan.recvFee/100 }"/>元</td>
				</tr>
				<tr>
					<td class="text-r">本次应收金额：</td>
					<td><input class="input w200"  readonly="readonly" style="background-color:#F0F0F0" value="${(collectionPlan.planCollectionFee-collectionPlan.recvFee)/100 }"/>元</td>
				</tr>
				
				<tr> 
					<td class="text-r">商户已缴押金:</td>
					<td><input class="input w200" id="payedDepositFee" style="background-color:#F0F0F0" readonly="readonly" value="${payedDepositFee }"/>元</td>
				</tr>
				<tr>
					<td class="text-r">本次应补押金:</td>
					<td><input class="input w200" id="depositFee" style="background-color:#F0F0F0" readonly="readonly" value="${shellDepositFee }"/>元	</td>
				</tr>
				<tr> 
