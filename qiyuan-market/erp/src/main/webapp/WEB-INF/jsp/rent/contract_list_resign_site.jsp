...
<script>
  ...
  function do_submit(){
    ...
    if($("#siteArea option:selected").attr('renttype')=='0'){
      ...
      if($("#leaseCount").val()==''){
        ...
        if(!reg.test($("#leaseCount").val())){
                ...
                if(parseFloat($("#leaseCount").val())>parseFloat($("#freeCount").val())){
    ...
    var data=$("#siteArea option:selected").attr('areaname')+";"+
    ...
    api.get('resignContractPage').doGetSitePage(data);
   	api.get("getResignSitePage",1).close();
