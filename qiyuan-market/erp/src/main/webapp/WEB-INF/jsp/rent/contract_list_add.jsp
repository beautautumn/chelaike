...
//		});
        ...
        <%--$("#agencyId").val("${contract.agency.id}");--%>
        ...
        <%--if("${agencyId}"){--%>
            <%--$('#agencyId').combobox("setValue","${agencyId}");--%>
        <%--}else{--%>
        	<%--$('#agencyId').combobox("setValue","${contract.agency.id}");--%>
        <%--}--%>
	...
	function openSiteArea(){
		...
		menu.create_child_dialog_identy(api,W,'getAddSitePage','选择场地区域',
    		'/rent/contractListAction!toAddSiteArea.action?areaNoAll='+areaNoAll+'&currentSiteAreas='+currentSiteAreas,
    		350,430,true);
    }
    ...
    function setEndDate(){
    	if(!$("#startDate").val()==''){
    		...
    		//setDate参数范围 1-31，now.setDate(now.getDate() - 1) 这样写让日期减一
    		//例如：当前日期为2016-01-01调用now.setDate(now.getDate() - 1)后 值为2015-12-31
	...
	function do_submit(){
		var agencyId=$("select[name='contractBean.agencyId']").val();
		...
		if(confirm("确认保存合同？")){
			...
			for(var i=1;i<$("#contractAreas tr").length;i++){
				...
				html7 = "<input type='hidden' name='contractAreas["+index+"].id' value='"+$("#contractAreas tr:eq("+i+") td:eq("+0+")").attr('contractAreaId')+"'/>";
			...
			$.ajax({
					cache: false,
					type: "POST",
					url:ctutil.bp()+"/rent/contractListAction!addContract.action",
					data:data,
					async: false,
					error: function() {
					html = "数据请求失败";
...
<div class="over-content">
	<form id="myForm">
		...
		<div class="over-cell">
			<label>合同有效期</label>
			<input type="text" class="over-date" id="startDate" name="contractBean.startDate" onblur="setEndDate()" value="<fmt:formatDate value="${contract.startDate}" pattern="yyyy-MM-dd"/>"/>
			<span class="poz">~</span>
			<input type="text" class="over-date"  id="endDate" name="contractBean.endDate"  onclick="WdatePicker()" value="<fmt:formatDate value="${contract.endDate}" pattern="yyyy-MM-dd"/>"/>
		</div>
		<%--<div class="over-cell">--%>
			<%--<label>凭证截图上传</label>--%>
	...
	<%--<div id="tab">--%>
		<%--<div class="box">--%>
			...
			<%--<input type="hidden" name="contractView" value="${contractView}"/>--%>
			<%--<input type="hidden" name="contractBean.id" id="contractId" value="${contract.id}"/>--%>
			...
			<%--<table>--%>
				<%--<tr>--%>
					...
					<%--<td width="400">--%>
						<%--<input class="easyui-combobox" style="width: 150px;" id="agencyId" name="contractBean.agencyId" />--%>
						<%--&lt;%&ndash; <select id="agencyId" class="input w100" name="contractBean.agencyId" >--%>
							<%--<s:iterator value="agencys">--%>
								<%--<option value="<s:property value='id'/>"  >--%>
									<%--${agencyName}--%>
								<%--</option>--%>
				...
				<%--<tr>	--%>
					<%--<td colspan="2">--%>
						<%--<table style="margin:auto; width:95%;margin-top:15px;" border='1' name="contractAreas" id="contractAreas">--%>
							...
							<%--<c:forEach items="${contractAreas}" var="contractArea">--%>
								<%--<tr align='center'>--%>
									<%--<td style='text-align:center;text-valign:center;width:50px;height:35px;' contractAreaId = '${contractArea.id}' monthrentfee = '${contractArea.monthRentFee/100}' id = '${contractArea.siteArea.id }' areaname='${contractArea.siteArea.areaName }' >${contractArea.siteArea.areaName }</td>--%>
									<%--<td style='text-align:center;text-valign:center;width:50px;height:35px;' renttype = '${contractArea.siteArea.rentType }' >${contractArea.siteArea.rentType }</td>--%>
									...
									<%--<td style='text-align:center;text-valign:center;width:50px;height:35px;' carcount = '${contractArea.carCount}' >${contractArea.carCount}</td>--%>
									<%--<td style='text-align:center;text-valign:center;width:50px;height:35px;' ><a onclick='deleteCurrentRow(this);'>删除</a></td>	--%>
								<%--</tr>--%>
							<%--</c:forEach>--%>
						<%--</table>--%>
				...
				<%--<tr>--%>
					...
					<%--<td width="400">--%>
						<%--<input type="text" style="width:120" id="startDate" name="contractBean.startDate" onblur="setEndDate();" value="<fmt:formatDate value="${contract.startDate}" pattern="yyyy-MM-dd"/>"/>--%>
						<%---<span class="red">*</span>--%>
						<%--<input type="text"  id="endDate" name="contractBean.endDate"  onclick="WdatePicker()" value="<fmt:formatDate value="${contract.endDate}" pattern="yyyy-MM-dd"/>"/>--%>
					<%--</td>--%>
				<%--</tr>--%>
				<%--&lt;%&ndash;<tr>--%>
					...
					<%--<td width="400">--%>
						...
						<%--<input type="text"  id="everyRecvFee" name="contractBean.everyRecvFee" style="background-color:#F0F0F0"  readonly="readonly" value="${contract.everyRecvFee}" />&nbsp;元--%>
					<%--</td>--%>
				<%--</tr>--%>
				<%----%>
				<%--<tr>--%>
					...
					<%--<td width="400">--%>
						<%--<input type="text" id="depositFee" style="width:120" name="contractBean.depositFee"  onblur="checkDepositFee();" value="${contract.depositFee/100 }" />&nbsp;元--%>
						<%--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;--%>
						<%--合同签订日：<span class="red">*</span>--%>
						<%--<input type="text"  id="signDate" name="contractBean.signDate"   style="background-color:#F0F0F0"  readonly="readonly" value="${contract.signDate }"  />--%>
					<%--</td>--%>
				<%--</tr>--%>
				<%--<tr>--%>
					<%--<td class="text-r" width="200px">市场签署人：<span class="red">*</span></td>--%>
					<%--<td width="400">--%>
						<%--<input type="text"  id="marketSignName" style="width:120" name="contractBean.marketSignName" value="${contract.marketSignName }"  />--%>
						<%--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;--%>
