...
<head>
    ...
    <script type="text/javascript">
        ...
        function ajaxPost(data2) {
            var url1 = ctutil.bp() + "/gateInfo/gateInfo!editGateInfo.action";
            $.ajax({
                cache: false,
                type: "POST",
                url: url1,
...
<div class="over-content">
    <form id="myForm" method="post" enctype ="multipart/form-data">
        <div class="over-cell">
            <label>道闸号</label>
            <input class="over-input" data-options="required:true,missingMessage:'请输入道闸号'" id="gateNum" name="gateInfoVO.no" value="${gateInfoVO.no}"/>
        </div>
        <div class="over-cell">
            <label>道闸名称</label><input class="over-input" style="width: 360px" maxlength="20" data-options="required:true,missingMessage:'请输入道闸名称'"  name="gateInfoVO.name" value="${gateInfoVO.name}"/>
        </div>
        <div class="over-cell">
            <label>出入方向</label>
            <select class="over-select" name="gateInfoVO.direction">
                <option value="">请选择</option>
                <option value="in" <c:if test="${gateInfoVO.direction == 'in'}">selected</c:if>>进场</option>
                <option value="out" <c:if test="${gateInfoVO.direction == 'out'}">selected</c:if>>出场</option>
            </select>
        </div>
        <input type="hidden" name="gateInfoVO.id" id="" value="${gateInfoVO.id}"/>
    </form>
</div>
</body>
</html>
