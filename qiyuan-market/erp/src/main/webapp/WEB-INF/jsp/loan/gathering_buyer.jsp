...
<script>
  ...
  $(document).ready(function(){
       $("#recvDate").val(new Date().format('yyyy-MM-dd'));
       //销售价格，第一次可以输入，以后只读
       ...
       if(salePrice-0>0)
       {
         ...
         $('#salePrice').css("background-color","#F0F0F0");       
       }
       calc();     
  }) 	
	...
	function calc(){
       ...
       var recvSaleFee =$('#recvSaleFee').val();
       if(recvSaleFee==null || recvSaleFee=='')
       {
         recvSaleFee =0; 
       }       
       //计算剩余应收款
       $("#curGatheringFee").val(salePrice-recvSaleFee);  
	...
	function ajaxPost(data){
		$.ajax({
				cache: false,
				type: "POST",
				url:ctutil.bp()+"/loan/loanEval!doGatheringBuyer.action",
				data:data,
				async: false,
				error: function() {
				  html = "数据请求失败";
...
<form id="myForm">
	...
	<div class="box">
      ...
      <tr>
        ...
        <td width="400px">
          <input class="input w200" id="salePrice" name="salePrice" value="${salePrice}" onblur="calc();"  onchange="if(isNaN(this.value)){alert('请输入合法数字!');this.value='';$('#salePrice').focus();}"/>万
        </td>
      </tr> 
      <tr>
        <td class="text-r" width="100px">
      ...
      <tr>
        ...
        <td width="400px">
          <input class="input w200" id="recvSaleFee" name="recvSaleFee"  style="background-color:#F0F0F0"  readonly="readonly"  value='${(recvSaleFee==null||recvSaleFee=="")?0:recvSaleFee}'    />万  
        </td>
      </tr> 
      <tr>
        <td class="text-r" width="100px">
        ...
        <td width="400px">
          <input class="input w200" id="recvSaleFee1" name="recvSaleFee1"  style="background-color:#F0F0F0"  readonly="readonly"  value='${salePrice-recvSaleFee}' />万
        </td>
      </tr>        
      <tr>
        <td class="text-r" width="100px">
        ...
        <td width="400px">
          <input class="input w200" id="curGatheringFee" name="curGatheringFee"   value='${salePrice-recvSaleFee}'   />万  
        </td>
      </tr> 
      <tr>
        <td class="text-r" width="100px">
        ...
        <td width="400px">
          <input name="financing.recvOverTag"  type="checkbox" value="0" onclick="checkValue(this);" />全部收完
        </td>
      </tr> 
	  ...
	  <tr>
				...
				<td width="400px">
					<select id="recvCustHis.staff.id" class="input w200" name="recvCustHis.staff.id">
      ...
      <tr style="width:200px;height:35px;">
          ...
          <td width="200px;">
            <input style="width:200px;height:25px;"  type="text"  id="recvDate" name="recvCustHis.recvDate"  onclick="WdatePicker()" />
          </td>
      </tr>
      <tr style="width:200px;height:90px;">
          <td class="text-r" width="200px;" valign="top" style="padding-top:10">收款说明: &nbsp;&nbsp;&nbsp;</td>       
          <td>
          <textarea name="recvCustHis.recvDesc" id="recvCustHis.recvDesc" style="width:200px;height:80px;"></textarea>
          </td>
      </tr> 
	</table>
	...
	<table style="margin:auto; width:95%;margin-top:15px;" border='0'>
		...
		<table style="margin:auto; width:95%;margin-top:15px;" border='1' name="contractAreas" id="contractAreas">
			<tr style="width:500px;">
				<td style="text-align:center;text-valign:center;width:50px;height:35px;"><font size="10px">收款人</font></td>
				<td style="text-align:center;text-valign:center;width:50px;height:35px;"><font size="10px">收款日期</font></td>
				<td style="text-align:center;text-valign:center;width:50px;height:35px;"><font size="10px">收款金额</font></td>
				<td style="text-align:center;text-valign:center;width:50px;height:35px;"><font size="10px">收款说明</font></td>
			</tr>
		  <c:if test="${empty recvCustHises}">
					<tr align="center">
						<td bgcolor="#ffffff">
							&nbsp;
						</td>
			...
			<c:if test="${not empty recvCustHises}">
				<c:forEach items="${recvCustHises}" var="op">
					<tr align="center">
					  <td bgcolor="#ffffff">
              ${op.staff.name}
            </td>
