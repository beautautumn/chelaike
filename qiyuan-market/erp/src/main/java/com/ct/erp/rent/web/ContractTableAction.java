...
@Scope("prototype")
@Controller("rent.contractTableAction")
public class ContractTableAction extends SimpleActionSupport  {
	...
	@Autowired
	private ContractAreaService contarctAreaService;
	...
	public String getContractAreaTable(){
		List<Contract> contracts = contractService.findAvailByAgencyId(agencyId);
			...
			for(Contract contract:contracts){
				List<ContractArea> contractAreas=this.contarctAreaService.findContractAreasByContractId(contract.getId());
	...
	public String getRecvDepositFee(){
		...
		for(Contract contract:contracts){
			JSONObject obj = new JSONObject();
			Integer recv_deposit_fee = (contract.getDepositFee()==null ? 0:contract.getDepositFee())-(contract.getPayedDepositFee()==null ? 0:contract.getPayedDepositFee());
			obj.put("recv_deposit_fee", UcmsWebUtils.fenToYuan(recv_deposit_fee>0?recv_deposit_fee:0));
			obj.put("colum", contract.getContractAreas().size());
			jsonArr.add(obj);
		}
		jsonStr = jsonArr.toString();
