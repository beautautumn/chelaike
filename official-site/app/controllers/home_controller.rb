...
class HomeController < ApplicationController
  ...
  def index
    ...
    request_params[:without_allied] = "true" if @tenant.tenant_type == "solo"
    result = Chelaike::CarService.query(request_params, @tenant.company_id)
    current_seller
    @cars_count = result.total
    @selling_cars = result.data

    return unless @tenant.tenant_type == "solo"
    info = alliance_main_site_info(@tenant.company_id)
    @alliance_name = info.alliance_name
    @alliance_cars_count = info.cars_count
  ...
  def load_onsale_cars
    ...
    if @tenant.tenant_type == "solo"
      request_params = request_params.merge(without_allied: false)
    end
    Chelaike::CarService.query(request_params, @tenant.company_id)
  end
