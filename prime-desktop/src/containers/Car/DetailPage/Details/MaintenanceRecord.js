...
import { Segment, UploadButton } from 'components'
...
import { update } from 'redux/modules/maintenanceRecords'
import styles from './MaintenanceRecord.scss'
...
export default class MaintenanceRecord extends Component {
  ...
  handleDelete = (id) => () => {
    ...
    const maintenanceData = this.props.maintenanceRecord || {}
    const maintenanceImages = maintenanceData.maintenanceImages
    if (id && maintenanceImages) {
      const newArray = []
      for (const image of maintenanceImages) {
        if (image.id === id) newArray.push({ ...image, _destroy: true })
      }
      update(car.id, newArray)
    }
  }

  render() {
    const { enumValues, oss, car } = this.props
    const maintenanceData = this.props.maintenanceRecord || {}
...
function MaintenanceImages({ oss, images, handleAdd, handleDelete }) {
  const imageCells = []
  if (images && images.length > 0) {
    imageCells.push(...images.map((image) => (
      <Col key={image.id} span="8" className={styles.imageCell}>
        ...
        <a className={styles.deleteButton} onClick={handleDelete(image.id)}>
  ...
  imageCells.push((
    <Col key="add-button" span="8" className={styles.uploadCell}>
      <UploadButton oss={oss} handleChange={handleAdd} />
    </Col>
  ))
  return (
    <Row>{imageCells}</Row>
...
function MonkeyKingAbstract({ record }) {
  if (!record || !record.carStatus) return false
