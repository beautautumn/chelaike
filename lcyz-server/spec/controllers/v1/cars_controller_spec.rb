...
RSpec.describe V1::CarsController do
  ...
  describe "POST /api/v1/cars" do
    ...
    it "creates a car" do
      ...
      expect(@car.owner_company).to eq tianche_peter
  ...
  describe "PUT /api/v1/cars/:id" do
      ...
      @request_lambda = lambda do
        auth_put :update, id: aodi.id,
                          car: {
                            ...
                            owner_company_id: tianche_allen.id
                          },
    ...
    it "更改归属车商" do
      ...
      @request_lambda.call
      expect(aodi.reload.owner_company_id).to eq tianche_allen.id
    end
  ...
  describe "GET /api/v1/cars" do
    ...
    context "lists all cars" do
      ...
      describe "出库列表" do
        ...
        context "根据出库类型进行过滤" do
          it "可以进行过滤" do
            aodi.update_columns(state: :sold)
            aodi.stock_out_inventories.create!(
              sales_type: :owner_company_retail,
              stock_out_inventory_type: :sold,
              current: true
            )

            auth_get :out_of_stock, query: { stock_out_inventory_sales_type_eq: "owner_company_retail" }
  ...
  describe "PUT batch_update_acquirer" do
    it "批量更新车源负责人" do
      give_authority(zhangsan, "车辆新增入库", "牌证信息录入", "车辆信息编辑")
      auth_put :batch_update_acquirer,
               car_ids: [aodi.id, a4_old.id],
               acquirer_id: lisi.id
      expect(aodi.reload.acquirer).to eq lisi
      expect(a4_old.reload.acquirer).to eq lisi
    end
  end
end
