module V1
  class CarsController < ApplicationController
    ...
    # 一辆车在入库前，检查vin码是否已存在
    def check_vin
      car = current_user.company.cars
                        .state_in_stock_scope
                        .where(vin: params[:vin].upcase)
    ...
    def batch_update_acquirer
      cars = Car.where(id: params[:car_ids])
      cars.update_all(acquirer_id: params[:acquirer_id])
    ...
    def car_params
      ...
      params.require(:car).permit(
        ...
        white_listed[:acquisition_type] = "consignment"
        ...
        # 把vin码转成大写
        white_listed[:vin] = params[:car][:vin].upcase
      end
    end
    ...
    def car_info_by_vin(vin)
      ...
      result = che3bai_service.model_by_vin(vin)
      ...
      model_info = result.fetch("modelInfo").last
