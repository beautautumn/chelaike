module V1
  class AntQueenRecordsController < ApplicationController
    before_action only: :index do
      authorize AntQueenRecord
    end
    skip_after_action :verify_authorized, only: %i(fetch refetch)

    def index
      query_params_validate
      records = AntQueenRecord.where(company_id: current_user.company_id)
                              .includes(:ant_queen_record_hub, :car)
      ...
      render json: records,
             each_serializer: AntQueenRecordSerializer::Basic,
             root: :data,
             meta: {
               balance: balance,
    ...
    def show
      ...
      record = AntQueenRecord.includes(:car, :ant_queen_record_hub).find(params[:id])
      ...
      if params[:only_app]
        ...
        render json: record,
               serializer: AntQueenRecordSerializer::Detail,
               root: :data
      end
    end
    ...
    def detail
      ...
      authorize AntQueenRecord, :detail?
      detail_fetch
      wrapper = CombinedSerializers.new(
        ant_queen_record: @record || AntQueenRecord.new,
        maintenance_images: @car.maintenance_images
      )
      render json: wrapper,
             serializer: AntQueenRecordSerializer::Wrapper,
             root: :data
    end
    ...
    def fetch
      param! :query, Hash, required: true do |q|
        ...
        q.param! :is_image, :boolean, default: true
        q.param! :engine_num, String
      end

      policy = AntQueenRecordPolicy.new(current_user, nil)
      return forbidden_error("您暂无维保查询权限") unless policy.fetch?

      vin = params[:query][:vin].upcase
      brand_id = params[:query][:brand_id]

      process_token(vin, brand_id) do |token|
        fetch_process(token)
      end
    rescue *AntQueen::FetchService::ERRORS => _e
      return render(status: 402, json: { message: "服务暂不可用，请稍后重试" }, scope: nil)
    end
    ...
    def refetch
      ...
      record = AntQueenRecord.find(params[:id])
    ...
    def warehousing
      ...
      render json: record,
             serializer: AntQueenRecordSerializer::Warehousing,
             root: :data
    end
    ...
    def process_token(vin, brand_id)
      ...
      token.with_lock do
        ...
        query_count = AntQueenRecord.where(vin: vin).count
        return render(status: 200, json: { data: { query_count: query_count } }, scope: nil)
      end
    end

    def detail_fetch
      @record = AntQueenRecord.find_by(car_id: params[:query][:car_id]) ||
                (@car.vin.presence &&
    ...
    def fetch_process(token)
      options = {
        ...
        is_text: params[:query][:is_text],
        engine_num: params[:query][:engine_num]
      }
      service = AntQueen::FetchService.new(params[:query][:vin],
                                           current_user,
                                           token,
                                           params[:query][:brand_id],
                                           options)
      service.execute
    end
    ...
    def refetch_process(record, token)
      ...
      service = AntQueen::FetchService.new(vin,
                                           current_user,
                                           token,
                                           record.car_brand_id,
                                           options)
