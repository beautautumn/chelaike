...
module CarSerializer
  class OpenDetail < CarSerializer::Open
    include ImageSerializer::Concern

    attributes :manufacturer_configuration, :configuration_note, :images, :prepare_items
    has_many :alliance_images, serializer: ImageSerializer::Common

    def prepare_items
      if !object.prepare_record || !object.prepare_record.prepare_items
        return nil
      end
      object.prepare_record.prepare_items.map do |item|
        {
          name: item.name,
          note: item.note
        }
    ...
    def images
      ...
      if alliance_company.present? && object.alliance_images.present?
        object.alliance_images.map do |image|
          image_attributes(image, image.url)
        end
      else
