  ...
  module Concern
    ...
    included do
      attributes :id, :company_id, :acquirer_id, :acquired_at, :shop_id,
                 ...
                 :is_onsale, :onsale_price_wan, :onsale_description, :owner_company_id

      attribute :acquisition_price_wan, if: :acquisition_price_wan?
      attribute :alliance_minimun_price_wan, if: :alliance_minimun_price_wan?
      attribute :sales_minimun_price_wan, if: :sales_minimun_price_wan?
      attribute :manager_price_wan, if: :manager_price_wan?

      attribute :sync_state_text
      attribute :owner_company_name
    end
    ...
    def attributes(requested_attrs = nil, reload = false)
      ...
      unless object.acquirer_id == scope.id
        attrs.except!(:cost_sum, :cost_statement) unless authority_filter("收购价格查看")
      end

      attrs.except!(:acquirer_id, :acquired_at) unless acquisition_info?

      unless stock_age_days?
        attrs.except!(
          :stock_age_days, :actual_states_statistic,
          :red_stock_warning_days, :yellow_stock_warning_days
        )
      end

      unless object.seller_id == scope.id
        attrs.except!(:closing_cost_wan) unless authority_filter("销售成交信息查看")

        unless authority_filter("车主寄卖信息查看")
          attrs.except!(:consignor_name, :consignor_phone, :consignor_price_wan)
        end
      end
