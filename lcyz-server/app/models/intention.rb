...
class Intention < ActiveRecord::Base
  ...
  scope :to_be_recycled, lambda { |recovery_hour|
    sql = <<-SQL.squish!
      intentions.state = 'failed'
      OR intentions.state = 'invalid'
      OR intentions.state = 'pending'
    ...
    with_customer.where(sql, time_limit: (Time.zone.now - recovery_hour.hours))
  }
  ...
  def to_be_recycled?
    recovery_hour = company.try(:intention_expiration).try(:recovery_hour)
    return if recovery_hour.to_i == 0

    state == "failed" || state == "invalid" || state_time_limit(state, recovery_hour)
  end

  def state_time_limit(state, recovery_hour)
    time_limit = (Time.zone.now - recovery_hour.hours)
  ...
  def push_histories_text
    texts = intention_push_histories.each_with_object([]) do |history, total_text|
      total_text << history_text(history)
    end
  ...
  def expired?
    ...
    (state == "processing" && processing_time < beginning_of_day) ||
      (state == "interviewed" && interviewed_time < now)
  end
  ...
  def history_text(history)
    executor_name = history.executor.name
    processing_time = history.processing_time.try(:strftime, "%Y-%m-%d %H:%M:%s")
    state_text = history.state_text
    "#{executor_name} 在 #{processing_time} 进行 #{state_text}"
  end
end
